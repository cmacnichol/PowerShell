...
$vComputerName = [Microsoft.VisualBasic.Interaction]::InputBox('Enter a computer name', 'Computer', "$env:computername") ## this is computer name using environment variable
$filepath = (Get-ChildItem env:\userprofile).value		## this is user profile  using environment variable

if ($vComputerName -eq '' -or $null) {Exit}
...
#  Huntington Software Tracking Keys
#ConvertTo-Html -Body "<H1>Huntington Software Registry Tracking Keys </H1>" >> "$filepath\$vComputerName.html"
$RegPath = "SOFTWARE\\HNB"
$REG = [Microsoft.Win32.RegistryKey]::OpenRemoteBaseKey('LocalMachine',$vComputerName)
$subkeys = $reg.OpenSubKey($RegPath).GetSubKeyNames()
$appList = $null
...
foreach($key in $SUBKEYS)
{
  ...
  $app = $REG.OpenSubKey($RegPath+"\\"+$key)
  $Release = $($app.GetValue("Release"))

                $tmpArray = New-Object -TypeName psobject
                $tmpArray | Add-Member -MemberType NoteProperty -Name "Application" -Value $key
                $tmpArray | Add-Member -MemberType NoteProperty -Name "Release" -Value $Release
                [Array]$appList += $tmpArray
}

$appList | ConvertTo-html -Head $a -Body "<H2>Huntington Software Registry Tracking Keys</H2>" >>  "$filepath\$vComputerName.html"
