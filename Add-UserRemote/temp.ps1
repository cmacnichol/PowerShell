...
Function Pause-At-End {
	while (((get-job | where-object {$_.State -eq "Running"}) | measure).Count -gt 0 -and $SyncTable.CancelScript -ne $true){
...
if ($SyncTable.UserList -eq $null){[System.Windows.Forms.MessageBox]::Show("User List in Empty.  Please enter at least one User ID." , "Missing User ID" , 0) | Out-Null; Exit}
	...
	if ($PopupAnswer -eq 1) {
		$compNameArray = $SyncTable.UserList
		...
		foreach ($user in $users.Members) {
				...
				if ($compNameArray -icontains $user) {$UsersToSkip += @($user)}
		}
		...
		foreach ($compName in $compNameArray) {
			...
			if ($UsersToSkip -contains $compName) {
				Log -Message "$compName already exists in Group!"
